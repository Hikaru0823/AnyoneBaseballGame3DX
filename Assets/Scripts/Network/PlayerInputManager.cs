...
public class PlayerInputManager : Fusion.Behaviour
{
    void Update()
	{
		if (PlayerObject.Local == null || PlayerObject.Local.Controller == null) return;
		if (GameManager.State.Current != GameState.EGameState.BeforeHit && GameManager.State.Current != GameState.EGameState.AfterHit) return;

		if (KeyAssignContent.Instance.Map["Base"].WasPerformedThisFrame())
		{
